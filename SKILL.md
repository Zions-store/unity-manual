...
| `StreamingAssets/` (root only) | Copied **verbatim** into the build, untouched by importers (configs, video, SQLite). Path: `Application.streamingAssetsPath`. Read-only at runtime; on Android/Web it's a URL — use `UnityWebRequest` |
